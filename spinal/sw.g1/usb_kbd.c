...
void drv_usb_kbd_init()
{
   UsbRegisterDriver(&KbdIf);
}
...
static int usb_kbd_translate(unsigned char scancode,unsigned char modifier,int pressed)
{
   ...
   LOG("scancode: 0x%x, modifier: 0x%x, pressed: 0x%x\n",scancode,modifier,pressed);
   if(pressed==0) {
      /* key released */
      repeat_delay=0;
      return 0;
...
int KeyboardClaim(uint8_t Adr,uint8_t *Buf)
{
   ...
   if(pIfDesc != NULL && ep != NULL && pDevDesc->bNumConfigurations == 1 && 
      pIfDesc->bInterfaceClass == 3 && pIfDesc->bInterfaceSubClass == 1 && 
